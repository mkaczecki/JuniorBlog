            ...
            <form method="POST" action="/posts/{{ $post->id }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <input name="title" placeholder="Post Title" class="form-control" id="title" type="text"
                           value="{{ old('title') ?? $post->title }}" class="@error('title') is-invalid @enderror" required>
                    @error('title')
                ...
                <textarea name="content" placeholder="Write here something amazing..."
                          ...
                          rows="20" style="resize: none;" required>{{ old('content') ?? $post->content }}
                </textarea>
                    @error('content')
                    <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                ...
                <button type="submit" class="btn btn-primary col-12 col-md-3 py-2 mt-3">Update</button>
            </form>
        </div>
    </div>
@stop
